...
#define NOTIFY_FORMAT(target, fmt)    if (format) { va_list args; va_start(args, fmt); vsnprintf(target, 4096, fmt, args); va_end(args); }
...
public:
    ...
    NOTIFY_INLINE auto set_title(const char *format, ...) -> void { NOTIFY_FORMAT(this->title, format)}

    NOTIFY_INLINE auto set_content(const char *format, ...) -> void { NOTIFY_FORMAT(this->content, format)}

    NOTIFY_INLINE auto set_type(const ImGuiToastType &toastType) -> void {
...
public:
    ...
    NOTIFY_INLINE auto get_color() const -> const ImVec4 {
        switch (this->type) {
            case ImGuiToastType_None:
...
public:
    ...
    ImGuiToast(ImGuiToastType type, const char *format, ...) : ImGuiToast(type) { NOTIFY_FORMAT(this->content, format)}

    ImGuiToast(ImGuiToastType type, int dismiss_time, const char *format, ...) : ImGuiToast(type, dismiss_time) {
        NOTIFY_FORMAT(this->content, format)
    }
};
        ...
        for (auto i = 0; i < notifications.size(); i++) {
            ...
            // Here we render the toast content
            {
                ...
                if (!NOTIFY_NULL_OR_EMPTY(title)) {
                    ...
                    Text("%s",
                         default_title); // Render default title text (ImGuiToastType_Success -> "Success", etc...)
                    was_title_rendered = true;
                }
